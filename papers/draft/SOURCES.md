...
| GKW16 Ch. 4.5 first published proof of the upper bound | `papers/REPORT-lit-1.md`, "George, Khodkar and Wallis (2016)..." paragraph |
...
| 41 | 6 | 47 | `search/gpu-41-5d.txt` ("NONE n=41 k=5 tested=17615450195") for the $k=5$ elimination; `search/ls-41-6.txt` ("WITNESS n=41 k=6 : (4,13) (28,30) (12,26) (17,31) (12,29) (24,27)") for the witness; cross-checked in `papers/REVIEW-search.md` ("(n=41)... 39 lengths = 3..41") |
...
| Cumulative `tested=17615450195` across resumed runs | `search/gpu-41-5d.txt`; resumption sequence visible in `search/hn_k5.csv` rows for $n=41$ (`ABORTED-no-output-processes-died`, then two `NONE` rows) |
...
| Griffin's own $k\le4$ elimination for $n\ge25$ (no new search needed for the $h(n)\ge5$ lower bound at $n=38..41$) | `papers/1312.0274.txt`, "Therefore, this in combination with our construction with 5 chords gives us knowledge of the values of m(n) for n<=37" and Corollary 1 ($2^{k+1}-1$ cycle ceiling); $k=4\Rightarrow$ ceiling $31$ |
...
| Consequence 1 (counting exactly balanced; $s\le2^k+1$) | `notes/01-subdivision-reformulation.md`, "Consequences" bullet 1 — unaffected by the correction |
...
| $t_k=2\,\mathrm{Fib}(k+3)-2$ for $k=2..5$, values $8,14,24,40$ | computed directly from the $t_k$ values above (`notes/01-subdivision-reformulation.md`) and the standard Fibonacci sequence; this specific closed form and the observation that it fits are new to this draft, not taken from any prior file — flagged in the draft text as "an observation, not a theorem" |
| Predicted $t_6=2\,\mathrm{Fib}(9)-2=66$ | same computation, extrapolated one step; explicitly labelled a prediction, not a verified value |
...
| GKW recipe pancyclic at exactly $n=36,40$, missing only length 5 on $[37,69]$ | `papers/REPORT-k6-upper.md`, "(a) Structured enumeration with geometric gaps" section; raw data `search/k6/gkw-K4.txt` |
...
* Every $n\ge38$ value in Section 2's table is backed by a witness/elimination
  file plus at least one of the three independent verifiers listed in Section
  3.4's table and cross-referenced in `papers/REVIEW-search.md`.
