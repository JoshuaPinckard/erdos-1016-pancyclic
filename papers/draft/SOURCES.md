...
| $t_6=67$ or $71\le t_6\le88$; $h(n)\le6$ for $41\le n\le67$; $h(n)\ge6$ for $n\ge66$ by counting | lower end and $h(n)\le6$: the Section 6.3 witness table below; upper end $t_6\le88$: the contiguous exhausted shape/CSP block $89\le n\le111$ — `search/shapecsp/level-k6-n94.txt` … `level-k6-n111.txt`, each `sat=0 gaveup=0`, plus the laptop descent's levels $93$ (114 eligible shapes, all UNSAT, receipt `REPORT-k6-level93-verify.md`), $92$ (152/152 UNSAT, `REPORT-k6-level92-verify.md`) and $91,90,89$ (all UNSAT, enumeration diff 0, same `bb` build, `REPORT-k6-level89-91-verify.md`; the run is documented in `papers/REPORT-k6-descent.md`), with $111$ the largest per-shape cap at $k=6$ (`search/shapecsp/shape-census.txt`, row `6 | 21878 | … | 109 (distinct 109)`, cap $=F+2$), analysed in `papers/REPORT-shape-csp.md`. Levels $68,69,70$ excluded: the GPU exhaustion of Section 3.6, `papers/REPORT-k6-gpu-pairwise-68-70.md` and the 21 claim files `papers/verification/n{68,69,70}-b{6..12}-{unrestricted,combined}.json` (zero hits, every claim `exact_match: true`). $N_0=2^{k+1}+1=129$ at $k=6$ (`papers/REPORT-cyclecounts.md`) is now only the trivial prior bound. The $0$-byte `search/shapecsp/level-k6-n92.txt` and `level-k6-n93.txt` are abandoned first attempts, not evidence. Counting step: $2^{5+1}-1=63<n-2$ iff $n\ge66$, recomputed here |
...
| `family_pancyclic_41_67 : ∀ n, 41 ≤ n → n ≤ 67 → PancyclicWithChords n 6`, 27 certificates, `lake build` clean (8743 jobs, no `sorry`), axioms `[propext, Classical.choice, Quot.sound]` for the range theorem and every certificate | `Erdos1016/Family.lean`; `Erdos1016/Family/W41.lean` … `W67.lean` (generated by `search/k6/gen_family_lean.py`, chords equal to $F_n$ of Section 6.3); `Axioms.lean` (35 theorems printed by `lake env lean Axioms.lean`, all three standard axioms); build run 2026-09-19 23:52 PDT by the scheduled task `Erdos1016-lean-build`, commit 5034d68 |
...
| $k=6$ ladder complete over $89\le n\le111$: levels $111..94$ `sat=0 gaveup=0`, then $93,92,91,90,89$ by the laptop descent | `search/shapecsp/level-k6-n94.txt` … `level-k6-n111.txt` (the aggregate `search/shapecsp/k6-levels.log` lacks the $94,95$ rows, which were run separately); descent receipts `REPORT-k6-level93-verify.md` (114/114 UNSAT, solver sha256 `e2fde07b…7f562c` equal to the block's `bb`), `REPORT-k6-level92-verify.md` (152/152 UNSAT) and `REPORT-k6-level89-91-verify.md` (every eligible shape at $91$ (181), $90$ (214) and $89$ UNSAT; `search/shapecsp/enum_eligible.py` re-enumeration diff 0), all read from the raw laptop files; `papers/REPORT-k6-descent.md` |

## Section 3.6 — GPU exhaustion of levels 68..70 (pairwise plan)

| Number / claim | Source |
|---|---|
| Levels $68,69,70$ exhausted, zero hits, every tier $b=6..12$ | `papers/REPORT-k6-gpu-pairwise-68-70.md` (tables generated from the claim files by `search/shapecsp/pairwise/render_claims.py`); 15 files `papers/verification/n{N}-b{6..10}-unrestricted.json` (`search/shapecsp/verify_tier_exhaustion.py`: unit sets re-derived from the census manifest, rank totals equal) and 6 files `n{N}-b{11,12}-combined.json` (`search/shapecsp/pairwise-prod/verify_tier_combined.py --rebuild -1`: every pairwise shape's tables rebuilt from its census row with matching hashes, counted compositions equal to the manifest totals) |
| Pairwise-admissible set $A$ contains every pancyclic composition; prune $42$–$47\times$ at $b=12$, $10$–$15\times$ at $b=11$ | `search/shapecsp/pairwise/SPEC.md` (necessity and monotonicity of `bound.hall_ok`); measured on the full $n=68..70$ manifests, `papers/REPORT-review-pairwise.md` and `papers/REPORT-k6-gpu-pairwise-68-70.md` ("Why the pairwise tiers are exact") |
| Census generator reproduces the $n=68,69,70$ manifests exactly | `search/shapecsp/pairwise/census_level.py`; recorded in `papers/REPORT-k6-gpu-pairwise-68-70.md` (Provenance) |
| Exact-set tests: 2,430,144 compositions equal to the reference recursion; kernel visits equal the CPU sequence, 27 family witnesses found; coverage carry bit-exact | `search/shapecsp/pairwise/test_pairwise_tables.py`, `test_pairwise_gpu.py`, `test_pairwise_carry.py`; runs recorded in `papers/REPORT-k6-gpu-pairwise-68-70.md` |
| Independent review: soundness of $A$, tables and kernel exact, forged-tier attacks D1–D4 rejected by the frozen claim tool | `papers/REPORT-review-pairwise.md`, addenda 1–5 |
| Blind level-67 control found $F_{67}$ at $b=11$, shape 20889, and nothing else | `papers/verification/control-n67-b12.json` (passed, 1675 units, 0 hits); `control-n67-b11.json` (written by `search/shapecsp/pairwise/check_control.py` when the tier completes; the hit itself is in `search/shapecsp/pairwise/control-state-n67-b11.json`, recorded with `search/verify.py pancyclic=True`, found 2026-09-20 00:19 PDT) |
| Frozen production code: byte-exact to commit f63f9f3 plus the one-line engine-capacity hotfix | `search/shapecsp/pairwise-prod/README.md`, `search/shapecsp/pairwise/freeze_prod.py`, `search/shapecsp/pairwise/test_engine_capacity.py` |
| Levels $71..88$ in progress; per-level unit counts | `papers/REPORT-k6-gpu-descent-71-88.md` (regenerated by `search/shapecsp/pairwise/update_report_descent.py`); tier manifests under `search/shapecsp/pairwise/tables-ext/` |
...
| Bracket $t_6=67$ or $71\le t_6\le88$ (the prior bound $129=2^7+1$ is superseded); surviving recurrence members $t_6\in\{72,74,\dots,88\}$, i.e. $-11\le t\le-3$ | lower end: the $n=67$ witness of Section 6.3 (`search/k6/witnesses-v2.csv`, `search/k6/verify-67-gpu-joint.txt`, re-run in `papers/draft/verify-rerun-20260914.txt`); upper end: the contiguous shape/CSP levels $n=89$ to $n=111$, all `sat=0 gaveup=0` (`search/shapecsp/level-k6-n94.txt` … `level-k6-n111.txt`; aggregate log `search/shapecsp/k6-levels.log` carries every row except $n=94,95$, which were run separately; levels $93,92,91,90,89$ from the descent receipts `REPORT-k6-level93-verify.md`, `REPORT-k6-level92-verify.md`, `REPORT-k6-level89-91-verify.md`); levels $68,69,70$: the Section 3.6 sources; arithmetic of the surviving members recomputed here ($66-2t<67$ iff $t\ge0$; $66-2t>88$ iff $t\le-12$; $t=-1,-2$ give $68,70$). The block, not any single level, is what establishes it: eligibility is decided at each cutoff by cap-and-Hall necessary conditions, which are not monotone in $n$, so a shape feasible at $n_0$ is refuted only by the level whose cutoff is exactly $n_0$ (`search/shapecsp/level.py` docstring, "Cite the block, never a single level"; the earlier mis-citation is recorded in `papers/REPORT-shape-csp.md` section 0a). $111$ is the largest cap over all $21878$ shapes, so no level above it is needed. The prior bound was $N_0=2^{k+1}+1$ from `papers/REPORT-cyclecounts.md`'s "Count-to-order comparison" table at $k=6$ |
...
| Inner-chord sweep $(1,a)(4,b)$ does not repair $n=68$; 2-of-6 sweep at $n=68$ complete, $36{,}481{,}725$ candidates, no witness; 3-of-6 was in flight | `papers/REPORT-k6-gpu-joint.md`, same section and "Neighbourhoods swept" table (row "68 ... 2 ... 36,481,725 ... 0"); superseded by the complete exhaustion of level $68$ (Section 3.6 sources) |
