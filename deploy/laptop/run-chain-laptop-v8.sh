#!/bin/bash
# Sequential chain for the laptop GPU.  Successor to run-chain-laptop-v7.sh
# (2026-09-19).  v1..v7 are left in place untouched: bash reads a script
# incrementally by byte offset.
#
# IDENTICAL to v7 in every job (68/11, 70/11, 70/12 resumed from their state
# files, then level 71 from pairwise/gpu-blast-ext + pairwise/tables-ext).  The
# one change: the runner and the modules it imports come from pairwise-prod/,
# a frozen byte-exact snapshot of pairwise/ at commit f63f9f3, so that a file
# copied into pairwise/ while a tier runs cannot be imported by the next tier.
# Tables, partitions and state files stay under pairwise/.
set -u
...
LOCK_WAIT=200
cd "$BASE" || exit 1
echo "[chain] start $(date -Is) script=v8 plan=pairwise runner=pairwise-prod"
rc=0
